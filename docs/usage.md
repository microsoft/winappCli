...
After migration, re-run only the deterministic checks at explicit workflow gates (for example, after project/resource edits or before final validation):
...
Nested projects and normal `ProjectReference` graphs are valid. `mechanicalVerification.targetProjectGraph` reports `not-required` when no ownership or portability check is required, `passed` when nested project ownership is protected by active exclusions/removals, `failed` for deterministic collisions or active target file items that use rooted machine-specific values or resolve outside the migration target, and `incomplete` when conditioned, property-expanded, imported, or unsupported MSBuild evidence prevents a safe conclusion. A `failed` or `incomplete` result creates verifier-owned `UWMIG013` and prevents `mechanical-migration-complete`. Resolve it by moving nested projects outside the entry project's default-item root or adding active exclusions/removals for their source and generated `obj`/`bin` content, and by copying required external files under the migration target and using target-relative item paths. Re-running `migrate verify` clears or reopens the issue and TODO from current evidence.
...
- `copied-linked-content` — available linked or external source `Content`/`PRIResource` was copied to an active explicit target `Include` and the source/target bytes match. A source MSBuild expression is accepted only when the CLI's bounded evaluator resolves it to exactly one existing file; unsupported, ambiguous, conditioned, or wildcard expressions remain review-required.
- `intentionally-not-migrated` — records the decision but deliberately leaves `UWMIG012` pending.

Evidence files must participate in the contained target build: the target project, an active automatically imported `Directory.Build.props`/`Directory.Build.targets`, or a file reached through active literal imports. Conditioned evidence is accepted only when the CLI can prove the condition from stable project facts. The migrated target must not retain file-item dependencies on the source checkout or machine-specific absolute paths; copy those files under the target root and reference them with target-relative paths. A rationale cannot override missing files, inactive imports/items, project-item metadata, path containment, or content equality. Later `migrate verify` rechecks every decision and reopens `UWMIG012` or `UWMIG013` if its evidence becomes stale. Report updates are serialized per target; if another `migrate` process holds the report transaction lock too long, retry after that process completes.
