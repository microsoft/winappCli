    ...
    ICurrentDirectoryProvider currentDirectoryProvider) : IMsixService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // correct PackagePaths, producing a cleaner MSIX without build artifacts. An explicitly
            // evaluated recipe path (AppxPackageRecipe) is preferred over a search of the input folder,
            // since the recipe can live outside the packaging output (e.g. under obj\).
            ...
            if (appxRecipe is not null)
            {
                if (!appxRecipe.Exists)
                {
                    throw new FileNotFoundException($"AppxPackageRecipe was not found: '{appxRecipe.FullName}'.");
                }
                recipeFile = appxRecipe;
            }
            else if (isMSBuildGenerated)
            {
