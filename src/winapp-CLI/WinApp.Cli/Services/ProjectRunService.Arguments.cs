...
internal sealed partial class ProjectRunService
{
    ...
    /// <summary>
    /// The effective build inputs as a property dictionary, for a later pass that has no way to accept
    /// them on its command line.
    /// </summary>
    /// <remarks>
    /// <c>dotnet package list</c> takes no <c>-c</c>, <c>-r</c> or <c>-p</c>, so package discovery would
    /// otherwise evaluate the app in the default configuration with no RID and none of the user's
    /// properties — a different dependency graph from the one that was just built. A
    /// <c>Directory.Build.props</c> that adds <c>Microsoft.WindowsAppSDK</c> only for <c>Release</c>, only
    /// for <c>win-arm64</c>, or only under a custom flag is then invisible, and the manifest's framework
    /// dependency and the runtime winapp provisions are decided from a graph that does not describe the
    /// binary. MSBuild reads environment variables as properties, which is how these are conveyed.
    /// <para>
    /// Ordering mirrors the evaluate pass: user <c>-p</c> first, then the inputs winapp owns, so the
    /// dedicated switches win exactly as they do there.
    /// </para>
    /// </remarks>
    internal static Dictionary<string, string> BuildEvaluationProperties(
        string configuration,
        string? runtimeIdentifier,
        string? targetFramework,
        string? platform,
        IEnumerable<string> forwardableProperties)
    {
        var evaluated = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);

        foreach (var property in forwardableProperties)
        {
            // A packed "A=1;B=2" is rejected before this point, so each token sets exactly one property.
            ...
            if (name.Length > 0)
            {
                evaluated[name] = property[(separator + 1)..].Trim();
            }
        }

        Set("Configuration", configuration);
        Set("RuntimeIdentifier", runtimeIdentifier);
        ...
        void Set(string name, string? value)
        {
            ...
            {
                evaluated[name] = value.Trim();
            }
        }
    }

    /// <summary>
    /// <see cref="BuildEvaluationProperties"/> for project mode, mirroring
    /// <see cref="BuildEvaluateArguments"/>'s filtering and injection rules.
    /// </summary>
    internal static Dictionary<string, string> BuildEvaluationProperties(ProjectRunOptions options) =>
        BuildEvaluationProperties(
            options.Configuration,
            options.OmitRuntimeIdentifier ? null : RunArchHelper.ToRuntimeIdentifier(options.Architecture),
            options.Framework,
            options.Platform,
            ForwardableProperties(options.Properties));

    /// <summary>
    /// <see cref="BuildEvaluationProperties"/> for single-file mode, mirroring
    /// <see cref="BuildSingleFileEvaluateArguments"/>. No <c>Platform</c>: a file-based app accepts it but
    /// does not use it for RID selection, so winapp never injects one.
    /// </summary>
    internal static Dictionary<string, string> BuildEvaluationProperties(SingleFileRunOptions options) =>
        BuildEvaluationProperties(
            options.Configuration,
            options.InjectedRuntimeIdentifier,
            targetFramework: null,
            platform: null,
            SingleFileForwardableProperties(options.Properties, options.InjectedRuntimeIdentifier is not null));
