...
internal partial class MsixService
{
    ...
    /// </summary>
    private const string RecipeLayoutStateFileName = ".winapp-recipe-files";
    ...
    {
        if (PathSafety.HasReparsePointOnExistingPath(outputDir.FullName))
        {
            throw new InvalidOperationException(
                $"The AppX output directory '{outputDir.FullName}' contains a symbolic link or junction.");
        }
        if (!outputDir.Exists)
        {
            outputDir.Create();
        ...
        var mappings = new Dictionary<string, FileInfo>(StringComparer.OrdinalIgnoreCase);
        var entries = recipeDoc.Descendants(msbuildNs + "AppXManifest")
        ...
        foreach (var entry in entries)
        {
            ...
            if (string.IsNullOrWhiteSpace(sourcePath) || string.IsNullOrWhiteSpace(packagePath))
            {
                throw new InvalidDataException(
                    $"The appx recipe '{recipeFile.FullName}' contains an entry without Include or PackagePath.");
            }

            var source = ResolveRecipeSource(recipeFile, sourcePath);
            if (!source.Exists)
            {
                throw new FileNotFoundException(
                    $"The appx recipe source for '{packagePath}' was not found: '{source.FullName}'.");
            }
            if (IsPathInsideDirectory(source.FullName, outputDir.FullName))
            {
                throw new InvalidDataException(
                    $"The appx recipe source '{source.FullName}' is inside its output directory.");
            }

            var destPath = ResolveRecipeDestination(outputDir, packagePath);
            if (!mappings.TryAdd(destPath, source) &&
                !string.Equals(mappings[destPath].FullName, source.FullName, StringComparison.OrdinalIgnoreCase))
            {
                throw new InvalidDataException(
                    $"The appx recipe maps more than one source to '{packagePath}'.");
            }
        }
    ...
    private static FileInfo ResolveRecipeSource(FileInfo recipeFile, string include)
    {
        try
        {
            var baseDirectory = recipeFile.DirectoryName
                ?? throw new InvalidDataException(
                    $"The appx recipe has no parent directory: '{recipeFile.FullName}'.");
            return new FileInfo(Path.GetFullPath(include, baseDirectory));
        }
        catch (Exception ex) when (ex is ArgumentException or NotSupportedException or PathTooLongException)
        {
            throw new InvalidDataException(
                $"The appx recipe source path '{include}' is invalid.",
                ex);
        }
    }
    ...
    private static string ResolveRecipeDestination(DirectoryInfo outputDir, string packagePath)
    {
        ...
        {
            throw new InvalidDataException(
                $"The appx recipe PackagePath '{packagePath}' resolves outside the output directory.");
        }
        if (PathSafety.HasReparsePointOnPath(destination, outputDir.FullName))
        {
            throw new InvalidDataException(
                $"The appx recipe PackagePath '{packagePath}' crosses a symbolic link or junction.");
        }
        if (string.Equals(
                Path.GetRelativePath(outputDir.FullName, destination),
                RecipeLayoutStateFileName,
                StringComparison.OrdinalIgnoreCase))
        {
            throw new InvalidDataException(
                $"The appx recipe PackagePath '{packagePath}' is reserved by WinApp.");
        }
        return destination;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return PathSafety.AtomicWriteAllTextAsync(
            Path.Join(outputDir.FullName, RecipeLayoutStateFileName),
            contents,
            new UTF8Encoding(encoderShouldEmitUTF8Identifier: false),
            cancellationToken);
    }

    private static void ValidateStagedEntryPoint(
        ...
        DirectoryInfo outputDirectory)
    {
        ...
        var executable = ResolveRecipeDestination(
            outputDirectory,
