 ...
 * 
 * @param {string} [startDir] - The directory to start searching from (defaults to current working directory)
 * @returns {string} The absolute path of the project root directory
 * @throws {Error} If no package.json is found
 */
function getProjectRootDir(startDir = process.cwd()) {
    ...
    while (true) {
        ...
        if (fs.existsSync(packageJsonPath)) {
            // During npm install, skip the windows-sdks package itself
            try {
                const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8'));
                if (packageJson.name === 'windows-sdks' && process.env.npm_config_prefix) {
                    // This is the windows-sdks package itself, keep looking up
