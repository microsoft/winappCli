...
function dotnet {
    ...
    if ([int]$minimum -ne $selected) { throw 'Missing exact minimum test threshold' }
...
<ResultSummary outcome="$outcome"><Counters total="$selected" executed="$selected" failed="$failed" /></ResultSummary>
</TestRun>
