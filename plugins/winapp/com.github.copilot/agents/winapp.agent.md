...
- `--aot` — (project mode) publish and run with .NET Native AOT. Requires effective `PublishAot=true`, supports x64/ARM64, and cannot use `--no-build`. Use `-c Release` for a Release AOT run.
