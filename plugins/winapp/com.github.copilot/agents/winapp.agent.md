...
- `--arch <x64|arm64|x86>` — (project mode) target architecture; default: current process arch. Sets the build RID and Windows App Runtime arch, and selects a matching platform-dependent publish profile when required. Honored in single-file mode too, where it defaults to the machine architecture.
