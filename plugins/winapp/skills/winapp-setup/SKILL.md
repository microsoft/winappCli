...
For parallel packaged checkouts, see `winapp-identity` and the canonical
...
- **The package outlives the run.** winapp says so the first time it registers an app. Remove it with `winapp unregister counter.cs` (automatically discovers normal or unique mode), or run with `--unregister-on-exit`. See [unregister](https://github.com/microsoft/WinAppCli/blob/main/docs/usage.md#unregister) for ambiguous layouts and source-deleted cleanup.
